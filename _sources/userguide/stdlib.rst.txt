...
Configures an MCP (Model Context Protocol) server using stdio transport. These 
are typically command-line tools installed as npm or Python packages that run 
as subprocesses. The task validates the command and optionally runs an 
installation command before use.
...
Produces
--------
Produces a ``std.AgentTool`` data item containing command and args for starting 
the MCP server.
...
* **args** - [Optional] List of command-line arguments to pass to the server

* **install_command** - [Optional] Command to run once before first use 
  (e.g., ``npm install``). Uses memento tracking to avoid re-running.

* **env** - [Optional] Map of environment variables to set when running the server

Error Handling
--------------

* **Command not found** → Task fails if command is not in PATH
* **Installation fails** → Task fails with stderr output from install command
* **Validation fails** → Task fails with appropriate error message

See Also
--------

See :doc:`mcp_tools` for comprehensive documentation, examples, and troubleshooting.


std.AgentToolHttp
=================

Configures an MCP (Model Context Protocol) server using HTTP/SSE transport. 
These are typically remote services or locally-hosted HTTP servers. The task 
validates URL format and optionally performs accessibility checks.
...
Produces
--------
Produces a ``std.AgentTool`` data item containing the URL for connecting to 
the MCP server.

Parameters
----------

* **url** - [Required] Base URL of the MCP server

* **validate** - [Optional] Whether to validate URL is accessible (default: false)

* **health_check_path** - [Optional] Path to append to URL for health check

* **headers** - [Optional] Map of HTTP headers to include in requests 
  (e.g., API keys)

* **timeout** - [Optional] Timeout in seconds for validation requests (default: 5)

Error Handling
--------------

* **Invalid URL** → Task fails if URL format is malformed
* **Connection failed** → Task fails if validate=true and URL is unreachable
* **Health check failed** → Task fails if health check returns non-2xx status
* **Timeout** → Task fails if request exceeds timeout duration
